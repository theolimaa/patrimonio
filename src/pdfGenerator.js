...
<div class="cover">
  ...
  <div class="cover-modules">
    ...
    <div class="cover-mod">
      ...
      <div class="cover-mod-desc">Consolidação do patrimônio inventariável e estimativa dos custos de inventário conforme estado civil e regime de bens.</div>
    </div>
    <div class="cover-mod">
...
<div class="section">
  ...
  <div class="section-desc">O planejamento sucessório visa organizar a transferência do patrimônio aos herdeiros da forma mais eficiente possível, minimizando os custos de inventário e garantindo que os bens cheguem ao destino certo. Consolidamos o patrimônio inventariável com base no estado civil do cliente e, quando aplicável, no regime de bens, estimando os custos totais do processo sucessório.</div>
  ${sucessao ? `
  <div class="nb"><table style="margin-bottom:12px;"><tr><th>Estado civil</th><td>${ESTADO_CIVIL[sucessao.estadoCivil] || '—'}</td></tr>${sucessao.estadoCivil === 'casado' || !sucessao.estadoCivil ? `<tr><th>Regime matrimonial</th><td>${REGIMES[sucessao.regimeCasamento] || '—'}</td></tr>` : ''}</table></div>
  ${sucessao.imoveis && sucessao.imoveis.some(function(im) { return im.valor }) ? (function() { const isCasado2 = !sucessao.estadoCivil || sucessao.estadoCivil === 'casado'; return `<div class="nb"><table><thead><tr><th>Imóvel</th><th>Tipo</th>${isCasado2 && sucessao.regimeCasamento === 'comunhao_parcial' ? '<th>Aquisição</th>' : ''}<th class="right">Valor</th><th class="right">Inventariável</th></tr></thead><tbody>${sucessao.imoveis.map(function(im, i) { const val = im.valor ? (parseInt(im.valor) / 100) : 0; const isCasado = !sucessao.estadoCivil || sucessao.estadoCivil === 'casado'; const frac = !isCasado ? 1 : sucessao.regimeCasamento === 'separacao_total' ? 1 : (sucessao.regimeCasamento === 'comunhao_universal' ? 0.5 : (im.antesCasamento ? 1 : 0.5)); return '<tr><td>Imóvel ' + (i+1) + '</td><td>' + (im.tipo||'—') + '</td>' + (sucessao.regimeCasamento === 'comunhao_parcial' ? '<td>' + (im.antesCasamento ? 'Antes' : 'Depois') + ' do casamento</td>' : '') + '<td class="right mono">' + fmtBRL(val) + '</td><td class="right mono">' + fmtBRL(val * frac) + '</td></tr>' }).join('')}</tbody></table></div>`; })()
   : ''}
  ${sucessao.veiculos && sucessao.veiculos.some(function(ve) { return ve.valor }) ? (function() { const isCasadoV = !sucessao.estadoCivil || sucessao.estadoCivil === 'casado'; return `<div class="nb"><table><thead><tr><th>Veículo</th><th>Tipo</th>${isCasadoV && sucessao.regimeCasamento === 'comunhao_parcial' ? '<th>Aquisição</th>' : ''}<th class="right">Valor</th><th class="right">Inventariável</th></tr></thead><tbody>${sucessao.veiculos.map(function(ve, i) { const val = ve.valor ? (parseInt(ve.valor) / 100) : 0; const frac = !isCasadoV ? 1 : sucessao.regimeCasamento === 'separacao_total' ? 1 : sucessao.regimeCasamento === 'comunhao_universal' ? 0.5 : (ve.antesCasamento ? 1 : 0.5); return '<tr><td>Veículo ' + (i+1) + '</td><td>' + (ve.tipo||'—') + '</td>' + (isCasadoV && sucessao.regimeCasamento === 'comunhao_parcial' ? '<td>' + (ve.antesCasamento ? 'Antes' : 'Depois') + ' do casamento</td>' : '') + '<td class="right mono">' + fmtBRL(val) + '</td><td class="right mono">' + fmtBRL(val * frac) + '</td></tr>' }).join('')}</tbody></table></div>`; })()
   : ''}
  ${(sucessao.patrimonioFinanceiro || 0) > 0 ? `<div class="nb"><table><thead><tr><th>Patrimônio Financeiro</th><th class="right">Valor total</th><th class="right">Inventariável</th></tr></thead><tbody>${(sucessao.previdenciaNum || 0) > 0 ? '<tr><td>Previdência privada (PGBL/VGBL)</td><td class="right mono">' + fmtBRL(sucessao.previdenciaNum) + '</td><td class="right mono" style="color:#15803d">Isento</td></tr>' : ''}<tr><td>Investimentos e aplicações <span style="font-size:10px;color:#8aa0b8;">(excl. previdência${sucessao.regimeCasamento !== 'separacao_total' ? ' · meação 50%' : ''})</span></td><td class="right mono">${fmtBRL((sucessao.patrimonioFinanceiro || 0) - (sucessao.previdenciaNum || 0))}</td><td class="right mono">${fmtBRL(((sucessao.patrimonioFinanceiro || 0) - (sucessao.previdenciaNum || 0)) * ((!sucessao.estadoCivil || sucessao.estadoCivil === 'casado') && (sucessao.regimeCasamento === 'comunhao_parcial' || sucessao.regimeCasamento === 'comunhao_universal') ? 0.5 : 1))}</td></tr></tbody></table></div>` : ''}
